...
workflow checks note and YouTube every Sunday at 09:17 JST. It commits stable
snapshots only when meaningful source content changes, then dispatches concept
generation. View, subscriber, like, and comment counts are excluded so those
volatile counters do not cause unnecessary OpenAI API requests.
